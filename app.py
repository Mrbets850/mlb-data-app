...
# STREAMLIT SECRETS
# -----------------------------
BATTERS_URL = st.secrets["BATTERS_URL"]
PITCHERS_URL = st.secrets["PITCHERS_URL"]
...
batters_df = standardize_columns(load_csv_from_url(BATTERS_URL))
pitchers_df = standardize_columns(load_csv_from_url(PITCHERS_URL))

if batters_df.empty:
    st.error("Batter CSV could not load. Check BATTERS_URL in secrets.")
if pitchers_df.empty:
    st.error("Pitcher CSV could not load. Check PITCHERS_URL in secrets.")
if batters_df.empty or pitchers_df.empty:
    st.stop()
...
with st.expander("Data status"):
    ...
    st.write("Batter URL loaded from Streamlit secrets.")
    st.write("Pitcher URL loaded from Streamlit secrets.")
